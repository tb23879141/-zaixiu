...
{
    public sealed unsafe class EQPushStream
    {
            ...
            AVFormatContext* vFormatCtx_In, AVCodecContext* pCodecCtx_h264, ref int video_index)
        {
            ...
            {
                result = ffmpeg.avformat_alloc_output_context2(f_pFormatCtx_Out, null, "flv", url);
                if (result < 0)
                {
        ...
        public int StartPush(string url, Action<Bitmap> camera_action)
        {
            ...
            #region open output
            if (OpenOutput(ref pFormatCtx_Out, url,
                    ...
                    try
                    {
                        ...
                        else
                        {
                            if (ffmpeg.av_read_frame(aFormatCtx_In, dec_pkt) >= 0)
                            {
                                ...
                                pframe = ffmpeg.av_frame_alloc();
                                pframe->channel_layout = ffmpeg.AV_CH_LAYOUT_STEREO;
            ...
            if (result == -10053)
            {
                HttpUtils.Instance.ShowTip("断网状态下无法正常结束直播，直播地址将被占用。", true);
            }
