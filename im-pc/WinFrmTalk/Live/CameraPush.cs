...
{
    public sealed unsafe class CameraPush
    {
        //Show Device  
        ...
        public int StartPush(string url, Action<Bitmap> camera_action)
        {
            ...
            if (ffmpeg.avformat_open_input(&ifmt_ctx, vdevice_name, ifmt, null) != 0)
            {
            ...
            while (ffmpeg.av_read_frame(ifmt_ctx, dec_pkt) >= 0)
            {
                ...
                if (dec_pkt->stream_index == videoIndex)
                {
                    ...
                    {
                        //展示到窗体中
                        using (var bitmap = new Bitmap(pframe->width, pframe->height, pframe->linesize[0], PixelFormat.Format24bppRgb, (IntPtr)pframe->data[0]))
                        {
        ...
        public int StartPush_Old(string url, Action<Bitmap> camera_action)
        {
            AVFormatContext* ifmt_ctx = null;
            AVFormatContext* ofmt_ctx;
            AVInputFormat* ifmt;
            ...
            string vdevice_name = "video=Logitech HD Webcam C270";
            string adevice_name = "audio=麦克风 (HD Webcam C270)";
            //sprintf(device_name, "video=screen-capture-recorder");
            ifmt = ffmpeg.av_find_input_format("dshow");
            //Set own video device's name
            if (ffmpeg.avformat_open_input(&ifmt_ctx, vdevice_name, ifmt, null) != 0)
            ...
            while (ffmpeg.av_read_frame(ifmt_ctx, dec_pkt) >= 0)
            {
                ...
                if (dec_pkt->stream_index == videoIndex)
                {
                    ...
                    if (dec_got_frame != 0)
                    {
                        ...
                        ffmpeg.sws_scale(img_convert_ctx, pframe->data, pframe->linesize, 0, pCodecCtx->height, pFrameYUV->data, pFrameYUV->linesize);
                ...
                else if (dec_pkt->stream_index == audioIndex)
                {
                    ...
                    AVStream* in_stream = ifmt_ctx->streams[dec_pkt->stream_index];
                    AVStream* out_stream = ofmt_ctx->streams[dec_pkt->stream_index];
                    var codec = ifmt_ctx->streams[dec_pkt->stream_index]->codec;
                    //dec_pkt->pts = ffmpeg.av_rescale_q_rnd(dec_pkt->pts, in_stream->time_base, out_stream->time_base, AVRounding.AV_ROUND_NEAR_INF | AVRounding.AV_ROUND_PASS_MINMAX);
                    //dec_pkt->dts = ffmpeg.av_rescale_q_rnd(dec_pkt->dts, in_stream->time_base, out_stream->time_base, AVRounding.AV_ROUND_NEAR_INF | AVRounding.AV_ROUND_PASS_MINMAX);
                    //dec_pkt->duration = ffmpeg.av_rescale_q(dec_pkt->duration, in_stream->time_base, out_stream->time_base);
                    //dec_pkt->pos = -1;

                    AVRational time_base = out_stream->time_base;//{ 1, 1000 };
                    //AVRational r_framerate1 = in_stream->r_frame_rate;// { 50, 2 };
                    AVRational time_base_q = new AVRational { num = 1, den = ffmpeg.AV_TIME_BASE };
                    //Duration between 2 frames (us)
                    //long calc_duration = Convert.ToInt64((double)(ffmpeg.AV_TIME_BASE) * (1 / ffmpeg.av_q2d(r_framerate1)));    //内部时间戳
                    //Parameters
                    //enc_pkt.pts = (double)(framecnt*calc_duration)*(double)(av_q2d(time_base_q)) / (double)(av_q2d(time_base));
                    dec_pkt->pts = ffmpeg.av_rescale_q(dec_pkt->pts, time_base_q, time_base);
                    dec_pkt->dts = dec_pkt->pts;
                    dec_pkt->duration = ffmpeg.av_rescale_q(dec_pkt->duration, time_base_q, time_base); //(double)(calc_duration)*(double)(av_q2d(time_base_q)) / (double)(av_q2d(time_base));
                    dec_pkt->pos = -1;

                    ret = ffmpeg.av_interleaved_write_frame(ofmt_ctx, dec_pkt);
                    //ret = ffmpeg.av_write_trailer(ofmt_ctx);
                }
