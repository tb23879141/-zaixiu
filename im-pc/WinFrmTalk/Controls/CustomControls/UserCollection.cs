...
namespace WinFrmTalk.Controls.CustomControls
{
    public partial class UserCollection : UserControl
    {
        ...
        #region 全局变量
        List<Collections> collectionShowlst = new List<Collections>();// 全部集合
        List<Collections> collectionlst = new List<Collections>();// 全部集合
        CollectionAdapter mAdapter;//保存适配
        ...
        public UserCollection()
        {
            InitializeComponent();
            LoadLanguageText();
            mAdapter = new CollectionAdapter();//适配绑定（保存）
            ...
            switch (data.type)
            {
                case "1"://kWCMessageType.Image
                    ...
                    else
                    {
                        ...
                        if (!File.Exists(imgfilePath))
                        {
                            DownloadEngine.Instance.DownUrl(data.url).Down((path) =>
                            {
                                StringCollection strcoll = new StringCollection();
                                strcoll.Add(path);
                                Clipboard.SetFileDropList(strcoll);
                ...
                case "3"://kWCMessageType.File
                    ...
                    //    Clipboard.SetFileDropList(strcoll);
                    //}
                    string filePath = Applicate.LocalConfigData.ImageFolderPath + FileUtils.GetFileName(data.url);
                    if (!File.Exists(filePath))
                    {
                        DownloadEngine.Instance.DownUrl(data.url).Down((path) =>
                        {
        ...
        {
            string url = CollectionItem.Collections.url;
            string downUrl = CollectionItem.Collections.msg;
            if (string.IsNullOrEmpty(downUrl))
            {
                Applicate.GetWindow<FrmMain>().ShowTip("文件不存在！");
                return;
