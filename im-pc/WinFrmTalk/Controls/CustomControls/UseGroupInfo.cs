...
namespace WinFrmTalk.Controls.CustomControls
{
    public partial class UseGroupInfo : UserControl
    {
        ...
        public UseGroupInfo()
        {
            ...
            tempLabel.ForeColor = Color.FromArgb(255, 49, 49);
            tempLabel.BackColor = System.Drawing.Color.Transparent;
            tempLabel.Font = new System.Drawing.Font("微软雅黑", 16F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Pixel);
            tempLabel.Name = "lab_temp";
            tempLabel.UseMnemonic = false;
        ...
        public void DisplayGroup(Friend group)
        {
            ...
            else if (group.GroupType == 4)
            {
                ...
                tempLabel.Location = new System.Drawing.Point(lblTitle.Location.X - 2, lblTitle.Location.Y + lblTitle.Height + 3);
                tempLabel.Text = string.Format("(此群{0})后自动解散", TimeUtils.FromatTime(group.deleteTime, "yyyy-MM-dd"));
            ...
            {
                if (group.movingState == 2)
                {
                    pictureBox1.Visible = true;
                    this.pictureBox1.Image = global::WinFrmTalk.Properties.Resources.groupGQZQ;
                }
                else
                {
                    pictureBox1.Visible = false;
            ...
            int locax = pictureBox1.Visible ? 125 : 87;
            lblTitle.Location = new Point(locax, lblTitle.Location.Y);
            ...
            HttpUtils.Instance.Get().Url(Applicate.URLDATA.data.apiUrl + "room/get") //获取群详情
                ...
                .AddParams("pageSize", "30")
                 ...
                 .Execute((sccess, result) =>
                 {
                     ...
                     if (type == 2)
                     {
                         var businessinfo = UIUtils.DecodeString(result, "businessinfo");
                         // 判断是否是官群
                         if (!UIUtils.IsNull(businessinfo))
                         {
                             var busne = JsonConvert.DeserializeObject<MyGroupGQ_Businessinfo>(businessinfo);
                             name = busne.name;
                         }
                     }

                     if (this.GroupInfo.NickName != name)
                     {
                         GroupInfo.NickName = name;
                         GroupInfo.UpdateNickName();
                         Messenger.Default.Send(GroupInfo, MessageActions.UPDATE_FRIEND_REMARKS);
                     ...
                     {
                         string member = UIUtils.DecodeString(result, "member");
                         var data1 = JsonConvert.DeserializeObject<MembersItem>(member);
                         GroupInfo.Role = Convert.ToString(data1.role);
                     }
                     else
                     {
                         GroupInfo.Role = "-1";
        ...
        private void AdddataTopal(List<MembersItem> memberlsts, int showMember)
        {
            ...
            // 群中群
            string roomJId = String.Empty;
            string roomName = String.Empty;
            int roomIndex = -1;

            for (int i = memberlsts.Count - 1; i > -1; i--)
            {
                var mem = memberlsts[i];
                if (!UIUtils.IsNull(mem.fromRoomId))
                {
                    roomJId = mem.fromRoomId;
                    roomName = mem.fromRoomName;
                    memberlsts.RemoveAt(i);
                    roomIndex = i;
                }
            }
            ...
            if (roomIndex > -1)
            {
                memberlsts.Insert(roomIndex, new MembersItem() { nickname = roomName, userId = roomJId, role = 5 });
            }
            ...
            for (int i = 0; i < memberlsts.Count; i++)
            {
                ...
                if (memberlsts[i].role == 5)
                {
                    uSEpicAddName.PicHead.Image = Resources.avatar_group;
                }
                else
                {
        ...
        private void Pics_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (uSEpicAddName.Userid.Length > 20)
                {
                    return;
                }
        ...
        /// <param name="e"></param>
        private void UseGroupInfo_SizeChanged(object sender, EventArgs e)
